...
int w = 10 ; //semakin kecil semakin dikit titik tengah,//batas threshold x,y //beda titik yg dekat2 akan menjadi satu titik, makin besar makin sidikit pasangan u,v
int t = 105; // batas threshold x_atas, y_atas, semakin kecil -- banyak pasangan atas-bawah-tengah
...
void cal_pt_to_go() {
  ...
  for (s = 1; s < j; s++) {
    ...
    for (n = 0; n <= p; n++) {
      if (( ((abs(x_tengah[s] - u[n])) < 100)  && ((abs(y_tengah[s] - v[n])) < 100) )) { //jika ya, maka
        goto b;
      }
    }
